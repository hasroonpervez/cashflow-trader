...
└── modules/
    ...
    ├── data.py               # Yahoo: `_ForcedTimeoutSession`, `YfData` timeout clamp; `fetch_global_market_bundle` / `GlobalMarketSnapshot` (**`rs_spy_ratio_map`**); **`rs_spy_ratio_map_from_close_matrix`**; `fetch_*` never-raise; tape + macro via desk slice
    ...
    ├── signal_desk.py        # **`compute_desk_consensus`**, **`traders_note_markdown`** (Unicorn when COIL+ICEBERG+LEADER; **`_recent_resistance_high`**), **`vwap_distance_stats`**, **`institutional_absorption`**, **`institutional_heatmap_ribbon_html`**, **`desk_conviction_multiplier`**, bento; **roadmap:** FFD momentum, correlation-aware sizer UX
...
2. **Context-first “bento” grouping** — **Shipped:** three columns under the note — **The setup** (Bollinger / squeeze language; **COIL** = BBW ≤ **5%** tile), **The momentum** (RSI, MACD, OBV, volume Z, **VWAP distance Z**, **batch RS vs SPY** line when present, optional **absorption / whale trap**), **The exit** (Gold Zone distance + copy).
...
6. **One-touch workflow** — **Shipped (illustrative):** expander **Calculate position size** — account $, **base risk %** × **`conviction_multiplier`** (**2.0×** only when **COIL + ICEBERG + SWEEP**; **Unicorn** note can still appear with **LEADER** without **SWEEP**, giving **1.25×**), **~1.5× ATR** stop below spot → **`suggested_shares_atr_risk`**; not broker-connected.
...
Covers `TA.get_correlation_matrix` (FFD-return path), earnings runway spark series, `Opt.portfolio_allocation` / `_simple_corr_haircut`, **`tests/test_signal_desk.py`** (consensus, absorption, VWAP Z, heatmap, **market leader**, **`test_traders_note_unicorn_perfect_storm`**), **`tests/test_data_rs.py`** (**`rs_spy_ratio_map_from_close_matrix`**), and basic Black–Scholes / EV math (no live Yahoo calls). Use **`python3 -m pytest`** if the `pytest` script is not on your `PATH`.
...
5. **Portfolio optimizer hook** — **Roadmap:** **correlation check** in the position-sizer path: warn or haircut when the candidate is highly correlated with symbols already treated as **held** or **scanned peers** (extends existing **`_simple_corr_haircut`** / cluster ideas to the desk UX).
