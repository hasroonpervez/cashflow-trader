...
- **Data layer** — `fetch_stock` uses `@st.cache_data(ttl=300)` (5 minutes). **`modules/data.py`** routes **yfinance** and direct Yahoo JSON fallbacks through a **single shared `curl_cffi` session** (`Session(impersonate="safari15_5", timeout=5.0)`), which **yfinance 0.2+ requires** (a stdlib `requests.Session` is rejected). Per-request caps use **`_YAHOO_YF_TIMEOUT`** (**5.0** seconds): every **`Ticker.history(...)`** and **`yf.download(...)`** passes **`timeout=_YAHOO_YF_TIMEOUT`**, because yfinance’s price scraper can otherwise wait **~10–30s** per call even when the session default is lower (**tar-pitting** shows up in logs as **`timed out after 30001 milliseconds`**). The same constant covers **intraday** history, **macro** strip pulls, and the **option-expiry** prime `history` call. **`retry_fetch`** retries **only on exceptions**, not on **empty** history, and **does not retry** read/timeouts so one bad response cannot stack into many seconds per ticker. The **watchlist tape** uses **one** `yf.download` batch per list (`watchlist_tape_pct_changes`, **120s** cache) instead of **N** per-symbol `history` calls. **Scan Watchlist** prints a **stderr** line when **SPY** is missing or empty so **RS vs SPY** is skipped for that scan without silent failure. Empty bars still log a **stderr** warning; **`yfinance`** may print **`possibly delisted`** for liquid tickers when Yahoo returns an empty payload — that is usually **egress throttling / bot scoring**, not a real delisting.
...
└── modules/
    ...
    ├── data.py               # yfinance + Yahoo HTTP: `_YAHOO_SESSION` + `_YAHOO_YF_TIMEOUT` on history/download; fetch_stock (300s); batched tape; macro
...
- **Yahoo Finance** may **throttle, block, or tar-pit** requests from **Streamlit Community Cloud** (shared IPs, bot heuristics). Symptoms: **empty** price data, **curl (28)** / **`Operation timed out after … ms`** in logs, scanner rows with **n/a**, and **`possibly delisted`** stderr from **`yfinance`** for symbols that still trade. **Reboot** the app for a new IP; reduce watchlist size and rerun frequency. The app uses **curl_cffi** (**Safari 15.5** impersonation), **5s** on the session **and** **`_YAHOO_YF_TIMEOUT`** on each **`history`** / **`download`**, plus **caching**, **batched tape downloads**, **no retry-on-empty**, and **no retry on timeout** — not a paid data feed. **Options** endpoints (e.g. **`option_chain`**) do not all expose the same **`timeout`** knob; price bars are the paths most hardened against long hangs.
