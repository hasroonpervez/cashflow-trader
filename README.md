...
- **Equity Radar & context-aware scanner** — **MISSION CONTROL → 🎛️ Command Center**: bordered **control deck** with two columns — **Trading Hemisphere** (**📈 Options Yield** vs **🎯 Equity Radar**, `segmented_control` with **radio** fallback) and **Capital Base** slider (Equity mode only). **Trading mode persists** to **`config.json`** as **`scanner_mode`** (restored on the next app launch; same atomic **`save_config`** path as the watchlist). The same **`Scan Watchlist`** run powers both modes; the UI **branches on `scanner_mode`** so **Equity Radar** does not surface options-specific columns and chrome (e.g. **GEX Regime**, Diamond **PoP**, **Scanner Data Table**) that belong on the premium-selling path. The **📝 Rolling Edge Capture Log** expander uses **mode-aware copy**: **Options Yield** explains premium-selling context (not a simple buy list); **Equity Radar** uses plain **“stocks to BUY”** language (including **IMMINENT BREAKOUT** as a direct buy-list framing). After **Scan Watchlist**, **📡 Radar Summary** HUD uses **`st.metric`** for **🔥 Imminent Breakouts**, **🟡 Accumulating**, and **Total Scanned** (reduces cognitive load before the grid). **🎯 Actionable Targets** table uses **string-formatted** currency / percent / thousands separators on **`Styler`**, row highlights unchanged, and **dynamic `st.dataframe` height** (`min(400, (rows+1)×38)`) to trim empty chrome. Core math and **`Opt.portfolio_allocation`** sizing are unchanged. **`scan_single_ticker`** accepts optional **`spy_df`**; **SPY** is fetched once per scan. If the radar UI path errors, a **caption** plus the **Options Yield** **Scanner Data Table** expander is shown.
...
**Intel → Risk, scanner & intel → Market Scanner:** toggling **📈 Options Yield** vs **🎯 Equity Radar** before or after **Scan Watchlist** switches the **scanner results presentation** (options desk vs equity radar table) on the next run; underlying per-ticker scan payloads are the same.
...
- **Market Scanner** output is produced in the script run that follows **Scan Watchlist**; other widget interactions trigger a full rerun — scan again if the scanner section looks empty
