...
└── modules/
    ...
    ├── signal_desk.py        # **`compute_desk_consensus`** (optional **`rs_spy_ratio`**, **`market_leader`**, conviction fields), **`vwap_distance_stats`**, **`institutional_absorption`**, **`institutional_heatmap_ribbon_html`**, **`desk_conviction_multiplier`**, trader’s note, bento; **roadmap:** FFD momentum, correlation-aware sizer UX
...
4. **Plain-English desk note** — **Shipped:** deterministic **`traders_note_markdown`** (markdown) below the consensus card — ATR-style stop line, squeeze / volume language, **VWAP stretch** copy when **|VWAP Z| ≥ 2**, **Market leader** copy when **`market_leader`**, and a dedicated **whale trap / absorption** paragraph when the absorption gate triggers (supersedes the generic “4σ footprint” line for that case). LLM swap remains a future option.
...
6. **One-touch workflow** — **Shipped (illustrative):** expander **Calculate position size** — account $, **base risk %** × **`conviction_multiplier`** (**1.0×–2.0×** from heatmap gates), **~1.5× ATR** stop below spot → **`suggested_shares_atr_risk`**; not broker-connected.
...
Covers `TA.get_correlation_matrix` (FFD-return path), earnings runway spark series, `Opt.portfolio_allocation` / `_simple_corr_haircut`, **`tests/test_signal_desk.py`** (consensus, absorption, VWAP Z, heatmap, **market leader**), **`tests/test_data_rs.py`** (**`rs_spy_ratio_map_from_close_matrix`**), and basic Black–Scholes / EV math (no live Yahoo calls). Use **`python3 -m pytest`** if the `pytest` script is not on your `PATH`.
