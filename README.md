...
- **Data layer** — `fetch_stock` uses `@st.cache_data(ttl=300)` (5 minutes). **`modules/data.py`** routes **yfinance** and direct Yahoo JSON fallbacks through a **single shared `curl_cffi` session** (`Session(impersonate="safari15_5", timeout=5.0)`), which **yfinance 0.2+ requires** (a stdlib `requests.Session` is rejected). The **5s** cap is applied on the **curl** session **and** on each **yfinance** ``history`` / ``download`` call (yfinance can use its own ~10–30s default if ``timeout`` is omitted). **`retry_fetch`** retries **only on exceptions**, not on **empty** history, and **does not retry** read/timeouts so one bad response cannot stack into many seconds per ticker. The **watchlist tape** uses **one** `yf.download` batch per list (`watchlist_tape_pct_changes`, **120s** cache) instead of **N** per-symbol `history` calls. **Scan Watchlist** logs to stderr when **SPY** cannot be loaded so **RS vs SPY** is skipped cleanly. Empty bars still log a **stderr** warning; **`yfinance`** may print **`possibly delisted`** for liquid tickers when Yahoo returns an empty payload — that is usually **egress throttling / bot scoring**, not a real delisting.
...
- **Yahoo Finance** may **throttle, block, or tar-pit** requests from **Streamlit Community Cloud** (shared IPs, bot heuristics). Symptoms: **empty** price data, **curl (28)** timeouts in logs, scanner rows with **n/a**, and **`possibly delisted`** stderr from **`yfinance`** for symbols that still trade. **Reboot** the app for a new IP; reduce watchlist size and rerun frequency. The app uses **curl_cffi** (**Safari 15.5** impersonation, **5s** HTTP timeout), **caching**, **batched tape downloads**, **no retry-on-empty**, and **no retry on timeout** so the UI does not hang on a single slow ticker — not a paid data feed.
