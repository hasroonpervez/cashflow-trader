...
- **Institutional heatmap ribbon + conviction sizer** — **Detailed desk** (non–mini mode): **`institutional_heatmap_ribbon_html`** after the trader’s note — **COIL** (purple, BBW percentile **≤ 5%**), **ICEBERG** (cyan, **`absorption`**), **SWEEP** (gold, **`vwap_urgency`**), **LEADER** (emerald, **`market_leader`**: **RS > 1** vs SPY **and** **volume Z > 4** on the desk’s 20d volume Z). Subtitle shows **`desk_conviction_multiplier`** tier (**1.0×** baseline, **1.25×** COIL and/or ICEBERG, **1.5×** SWEEP only, **2.0×** when **all three** microstructure gates **COIL + ICEBERG + SWEEP** align — **not** the same condition as **LEADER** alone). **Calculate position size** expander applies **base risk % × conviction multiplier** to **`suggested_shares_atr_risk`** (illustrative only).
...
└── modules/
    ...
    ├── data.py               # Yahoo: `_ForcedTimeoutSession`, `YfData` timeout clamp; **`fetch_stock`** → optional **Alpha Vantage** daily fallback when **`ALPHAVANTAGE_API_KEY`** set; `fetch_global_market_bundle` / `GlobalMarketSnapshot` (**`rs_spy_ratio_map`**); **`rs_spy_ratio_map_from_close_matrix`**; `fetch_*` never-raise; tape + macro via desk slice
    ...
    ├── signal_desk.py        # **`compute_desk_consensus`** (+ **`unified_probability`**, **`ofi_detail`**, **`daily_aggressor_proxy`**, **`blend_unified_probability`**), **`unified_probability_dial_html`**, **`traders_note_markdown`** (Unicorn when COIL+ICEBERG+LEADER; **`_recent_resistance_high`**), **`vwap_distance_stats`**, **`institutional_absorption`**, **`institutional_heatmap_ribbon_html`**, **`desk_conviction_multiplier`**, **`bento_box_html`**, **`bento_accents_from_consensus`**; **roadmap:** FFD-weighted momentum
...
3. **Rolling VWAP distance Z** — **Shipped:** **`vwap_distance_stats`**: **20-bar** rolling VWAP, **Z** of relative close-vs-VWAP deviation vs **prior 20** bars; consensus **flow** blend, **`vwap_z`**, **`vwap_detail`**, **`vwap_urgency`**; trader’s note when **|Z| ≥ 2**; **SWEEP** segment on heatmap when **`vwap_urgency`**.
4. **Batch RS vs SPY + market leader** — **Shipped:** **`rs_spy_ratio_map_from_close_matrix`** in **`data.py`**; **`compute_desk_consensus(..., rs_spy_ratio=)`**; **`market_leader`** and **LEADER** ribbon when **RS > 1** and **volume Z > 4**.
5. **Portfolio optimizer hook** — **Partially shipped:** **ρ > 0.75** vs active ticker warning after the heatmap; **roadmap:** haircut inside the sizer when **held** symbols are modeled explicitly.
