...
- **Data layer** — `fetch_stock` uses `@st.cache_data(ttl=300)` (5 minutes). **`modules/data.py`** routes **yfinance** and direct Yahoo JSON fallbacks through a **single shared `curl_cffi` session** subclass (**`_ForcedTimeoutSession`**) with **`impersonate="safari15_5"`**, which **yfinance 0.2+ requires** (a stdlib `requests.Session` is rejected). The subclass overrides **`request()`** so **`kwargs["timeout"]`** is always **`_YAHOO_YF_TIMEOUT`** (**5.0** seconds), overriding yfinance internal **~30s** timeouts on **`.info`**, **`.options`**, and similar paths (including **10x** sieve **`fetch_info`** and chains). Every **`Ticker.history(...)`** and **`yf.download(...)`** also passes **`timeout=_YAHOO_YF_TIMEOUT`** for the price scraper. **Tar-pitting** may still log **`timed out after … milliseconds`**, but each HTTP hop is capped. **`retry_fetch`** retries **only on exceptions**, not on **empty** history, and **does not retry** read/timeouts. The **watchlist tape** uses **one** `yf.download` batch per list (`watchlist_tape_pct_changes`, **120s** cache). **Scan Watchlist** prints **stderr** when **SPY** is missing or empty so **RS vs SPY** is skipped. Empty bars log a warning; **`possibly delisted`** on liquid names is usually **throttling**, not delisting.
