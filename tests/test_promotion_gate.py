...
def test_gate_promotes_balanced_sample():
    # 40 alternating hits, two labels evenly
    outcomes = [1.0, 0.0] * 20
    labels = (["A", "B"] * 20)
    d = promotion_gate(
        outcomes,
        min_n=30,
        split_half_min=-1.0,  # allow weak corr for this unit test
...
def test_gate_concentration():
    outcomes = [1.0] * 40
    labels = ["ONLY"] * 40
    d = promotion_gate(outcomes, min_n=30, split_half_min=-1.0, max_concentration=0.5, labels=labels)
    assert d.promote is False
