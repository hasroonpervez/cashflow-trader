"""Coinbase venue stub — paper noop / live refused."""
...
class CoinbasePaperAdapter(VenueAdapter):
    name = "coinbase"
    ...
    def __init__(self, mode: Mode | str = Mode.PAPER) -> None:
        ...
        if mode_obj is Mode.LIVE:
            raise ValueError("CoinbasePaperAdapter: live not enabled (dual-OK required)")
    ...
    def _place_paper(self, order: OrderRequest) -> Fill:
        return Fill(
            order_id=f"cb-paper-{order.signal_id or 'na'}",
            market=order.market,
            side=order.side,
            size=float(order.size),
            price=float(order.price or 0.0),
            mode=self.mode,
            raw={"adapter": self.name, "stub": True},
        )
